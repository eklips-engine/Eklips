...
class Tileset(Object):
    ...
    def tiles(self, value):
        self._sprites = {}
        for i in value:
            self.add_tile(*value[i])
    
    def add_tile(self, sx, sy, sw, sh, hx, hy, hw, hh):
        self._sprites[len(self._sprites)] = [sx, sy, sw, sh, hx, hy, hw, hh]
        self.refresh_tiles()
    
    def refresh_tiles(self):
        ogsprites     = self._sprites.copy()
        for i in ogsprites: #[SX, SY, SW, SH, HX, HY, HW, HH, img if loaded]
            self._sprites[i][8] = self.image.get_region(*self._sprites[:4])
